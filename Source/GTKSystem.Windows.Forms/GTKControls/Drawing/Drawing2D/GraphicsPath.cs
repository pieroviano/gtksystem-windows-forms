...
public sealed class GraphicsPath : MarshalByRefObject, ICloneable, IDisposable
{
    ...
    public byte[] PathTypes { get; private set; }
    ...
    /// <param name="types">An array of <see cref="T:System.Drawing.Drawing2D.PathPointType" /> enumeration elements that specifies the type of each corresponding point in the <paramref name="pts" /> array.</param>
    /// <param name="fillMode">A <see cref="T:System.Drawing.Drawing2D.FillMode" /> enumeration that specifies how the interiors of shapes in this <see cref="T:System.Drawing.Drawing2D.GraphicsPath" /> are filled.</param>
    public GraphicsPath(Point[] pts, byte[] types, FillMode fillMode) : this(
        Array.ConvertAll<Point, PointF>(pts, o => new PointF(o.X, o.Y)), types, fillMode)
    {
    }
    ...
    public void AddArc(Rectangle rect, float startAngle, float sweepAngle)
    {
        ...
        list.Add(new ArcMode()
        {
            rect = new RectangleF(rect.X, rect.Y, rect.Width, rect.Height), startAngle = startAngle,
    ...
    public void AddArc(int x, int y, int width, int height, float startAngle, float sweepAngle)
    {
        ...
        list.Add(new ArcMode()
            { rect = new RectangleF(x, y, width, height), startAngle = startAngle, sweepAngle = sweepAngle });
    }
    ...
    {
        AddPathPoints(new PointF(pt1.X, pt1.Y), new PointF(pt2.X, pt2.Y), new PointF(pt3.X, pt3.Y),
            new PointF(pt4.X, pt4.Y));
        list.Add(new BezierMode()
        {
            pt1 = new PointF(pt1.X, pt1.Y), pt2 = new PointF(pt2.X, pt2.Y), pt3 = new PointF(pt3.X, pt3.Y),
            pt4 = new PointF(pt4.X, pt4.Y)
        });
    }
    ...
    public void AddBezier(int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4)
    {
        ...
        list.Add(new BezierMode()
        {
            pt1 = new PointF(x1, y1), pt2 = new PointF(x2, y2), pt3 = new PointF(x3, y3), pt4 = new PointF(x4, y4)
        });
    }
    ...
    {
        AddPathPoints(Array.ConvertAll(points, p => new PointF(p.X, p.Y)));
        list.Add(new ClosedCurveMode()
            { points = Array.ConvertAll(points, p => new PointF(p.X, p.Y)), tension = tension });
    }
    ...
    {
        AddPathPoints(points);
        list.Add(new CurveMode()
            { points = points, offset = offset, numberOfSegments = numberOfSegments, tension = tension });
    }
    ...
    public void AddCurve(Point[] points, int offset, int numberOfSegments, float tension)
    {
        ...
        list.Add(new CurveMode()
        {
            points = Array.ConvertAll(points, p => new PointF(p.X, p.Y)), offset = offset,
            numberOfSegments = numberOfSegments, tension = tension
        });
    }
    ...
    public void AddRectangles(Rectangle[] rects)
    {
        foreach (RectangleF rect in rects)
            AddPathPoints(new PointF(rect.Left, rect.Top), new PointF(rect.Right, rect.Bottom));
        list.Add(new RectanglesMode()
            { rects = Array.ConvertAll(rects, r => new RectangleF(r.X, r.Y, r.Width, r.Height)) });
    }
    ...
    public void AddString(string s, FontFamily family, int style, float emSize, Point origin, StringFormat format)
    {
        ...
        list.Add(new StringMode()
        {
            text = s, family = family, style = style, emSize = emSize,
            layoutRect = new RectangleF(origin.X, origin.Y, 0, 0), format = format
        });
    }
        ...
        StringFormat format)
    {
        ...
        {
            text = s, family = family, style = style, emSize = emSize,
            layoutRect = new RectangleF(layoutRect.X, layoutRect.Y, layoutRect.Width, layoutRect.Height),
            format = format
        });
    }
