...
public sealed class DeserializingResourceReader : System.Resources.IResourceReader, IEnumerable, IDisposable
{
            ...
            get
            {
                ...
                lock (_reader)
                {
                    lock (_reader._resCache)
                    {
                        ...
                        {
                            obj = ((_dataPosition != -1)
                                ? _reader.LoadObject(_dataPosition)
                                : _reader.GetValueForNameIndex(_currentName));
                        }
                    }
                }
    ...
    public DeserializingResourceReader(string fileName)
    {
        ...
        _store = new BinaryReader(
            new FileStream(fileName, FileMode.Open, FileAccess.Read, FileShare.Read, 4096,
                FileOptions.RandomAccess), Encoding.UTF8);
        try
        {
            ReadResources();
        }
    ...
    private unsafe string AllocateStringForNameIndex(int index, out int dataOffset)
    {
        ...
        lock (this)
        {
            ...
            while (num3 > 0)
            {
                ...
                if (num4 == 0)
                {
                    throw new EndOfStreamException(SR.Format(SR.BadImageFormat_ResourceNameCorrupted_NameIndex,
                        index));
                }
        ...
        else
        {
            ...
            if (resourceTypeCode != ResourceTypeCode.String && resourceTypeCode != 0)
            {
                throw new InvalidOperationException(SR.Format(
                    p1: (resourceTypeCode >= ResourceTypeCode.StartOfUserTypes)
                        ? FindType((int)(resourceTypeCode - 64)).FullName
                        : resourceTypeCode.ToString(), resourceFormat: SR.InvalidOperation_ResourceNotString_Type));
            }
        ...
        switch (typeCode)
        {
            ...
            case ResourceTypeCode.ByteArray:
            {
                ...
                if (_ums == null)
                {
                    ...
                    {
                        throw new BadImageFormatException(SR.Format(SR.BadImageFormat_ResourceDataLengthInvalid,
                            num2));
                    }
    ...
    {
        if (TypeNameComparer.Instance.Equals(readerType,
    ...
    private unsafe object DeserializeObject(int typeIndex)
    {
        ...
        if (obj.GetType() != type)
        {
            throw new BadImageFormatException(SR.Format(SR.BadImageFormat_ResType_SerBlobMismatch, type.FullName,
