...
internal sealed class TypeNameComparer : IEqualityComparer<string>
{
    ...
    public bool Equals(string assemblyQualifiedTypeName1, string assemblyQualifiedTypeName2)
    {
        ...
        readOnlySpan = ((readOnlySpan.Length > span.Length)
            ? readOnlySpan.Slice(span.Length + 1).TrimStart(s_whiteSpaceChars)
            : ReadOnlySpan<char>.Empty);
        readOnlySpan2 = ((readOnlySpan2.Length > other.Length)
            ? readOnlySpan2.Slice(other.Length + 1).TrimStart(s_whiteSpaceChars)
            : ReadOnlySpan<char>.Empty);
    ...
    {
        ReadOnlySpan<char> assemblyQualifiedTypeName2 =
            assemblyQualifiedTypeName.AsSpan().TrimStart(s_whiteSpaceChars);
