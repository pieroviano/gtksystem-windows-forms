...
internal class MainForm {
    private static readonly Array style_values = Enum.GetValues(typeof(ControlStyles));
    private static readonly string[] style_names = Enum.GetNames(typeof(ControlStyles));

    private static readonly string TestHeader =	"//\n" +
    ...
    private static readonly string TestFooter =	"\t}\n}\n";

    public static string[] GetStyles(Control control) {
        string[] result = new string[style_names.Length];

        for (var i = 0; i < style_values.Length; i++) {
            result[i] = style_names[i] + "=" + control.GetType().GetMethod("GetStyle", BindingFlags.NonPublic | BindingFlags.Instance)!.Invoke(control,
                [(ControlStyles)style_values.GetValue(i)!]);
        }
    ...
    public static void TestStyles(StreamWriter file, Control control, string name) {
        ...
        file.WriteLine("\t\t\t};\n");
        file.WriteLine("\t\t\tAssert1.AreEqual({0}_want, GetStyles(new {0}()), \"{0}Styles\");", name);
        file.WriteLine("\t\t}\n\n");
    }
