...
public class TestHelper
{
    ...
    public class FormWatcher : Form
    {
        ...
        protected override void WndProc(ref Message m)
        {
            foreach (var i in watches)
            {
                if (m.Msg == i)
                {
                    Console.WriteLine((Name != string.Empty && Name != null ? Name : "FormWatcher") + " received message " + m.ToString());
                    break;
                }
            }
            base.WndProc(ref m);
    ...
    public class ControlWatcher : Control
    {
        ...
        protected override void WndProc(ref Message m)
        {
            foreach (var i in watches)
            {
                if (m.Msg == i)
                {
                    Console.WriteLine((Name != string.Empty && Name != null ? Name : "ControlWatcher") + " received message " + m.ToString());
                    break;
                }
            }
            base.WndProc(ref m);
    ...
    public class ButtonWatcher : Button
    {
        ...
        protected override void WndProc(ref Message m)
        {
            foreach (var i in watches)
            {
                if (m.Msg == i)
                {
                    Console.WriteLine((Name != string.Empty && Name != null ? Name : "ButtonWatcher") + " received message " + m.ToString());
                    break;
                }
            }
            base.WndProc(ref m);
    ...
    public static void DumpObject(object obj, Type objType, string objName, StringWriter writer, int tabs, string prefix, ArrayList done, int maxrecursive, int level)
    {
        ...
        for (var j = 0; j < sorted.Length; j++)
        {
            ...
            switch (Type.GetTypeCode(value.GetType()))
            {
                ...
                case TypeCode.Object:
                    ...
                    if (value is Exception)
                    {
                        ...
                        writer.WriteLine(tab + "\tAssert1.AreEqual(@\"{0}\", ex.Message);", ((Exception)value).Message.Replace("\"", "\"\""));
            ...
            writer.WriteLine("{0}Assert1.AreEqual({4}, {1}.{2}, \"{3}\");", tab, showName, name, message, code);
        }
    }
